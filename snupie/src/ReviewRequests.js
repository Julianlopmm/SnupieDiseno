...
function ReviewRequests() {
  ...
  useEffect(() => {
    ...
    fetch('https://api-snupie-acs-287732579337.us-central1.run.app/solicitudes/pendientes')
      .then(response => {
        if (!response.ok) {
  ...
  const handleAprobarSolicitud = (id) => {
    fetch(`https://api-snupie-acs-287732579337.us-central1.run.app/solicitud/aprobar/${id}`, {
      method: 'PUT',
    })
      .then(response => {
        if (!response.ok) {
  ...
  const handleRechazarSolicitud = (id) => {
    fetch(`https://api-snupie-acs-287732579337.us-central1.run.app/solicitud/rechazar/${id}`, {
      method: 'PUT',
    })
      .then(response => {
        if (!response.ok) {
